  ...
  return <div className="min-h-screen bg-background">
      ...
      <div className="bg-background border-b border-border p-4">
        ...
        <div className="relative">
          ...
          <input type="text" placeholder="Search leads by name or phone number" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onFocus={handleSearchFocus} onBlur={handleSearchBlur} className="w-full pl-10 pr-10 py-2 bg-muted/30 rounded-xl border border-input text-center placeholder:text-center focus:outline-none focus:ring-2 focus:ring-ring" />
          {searchQuery && <button onClick={clearSearch} className="absolute right-3 top-1/2 transform -translate-y-1/2">
              <X className="h-4 w-4 text-muted-foreground" />
            </button>}
          
          {/* Autocomplete Dropdown */}
          {showAutocomplete && <SearchAutocomplete leads={searchQuery ? filteredLeads : leadsData} onLeadSelect={handleLeadSelect} searchQuery={searchQuery} />}
        </div>
      </div>
      ...
      <div className="p-6 space-y-6">
        ...
        {/* Current Lead Card */}
        <Card className="shadow-2xl border-border/50 rounded-3xl bg-card px-[16px]">
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-4">
            <Button variant="outline" onClick={handlePrevious} disabled={historyIndex <= 0} className="flex-1 h-12 rounded-2xl shadow-lg">
              <ArrowLeft className="h-4 w-4 mr-2" />
              Previous
            </Button>
            
            <Button variant="outline" onClick={handleNext} disabled={filteredLeads.length <= 1} className="flex-1 h-12 rounded-2xl shadow-lg">
              Next
          ...
          <div className="flex items-center justify-between">
            <div className="flex flex-col items-center space-y-1 flex-1">
              <button onClick={toggleShuffle} disabled={filteredLeads.length <= 1} className="p-3 rounded-full transition-colors disabled:opacity-50">
            ...
            <div className="flex flex-col items-center space-y-1 flex-1">
              <button onClick={toggleAutoCall} className={`text-sm font-medium transition-colors px-3 py-1 rounded ${autoCall ? 'text-green-600' : 'text-muted-foreground'}`}>
                Auto Call
              </button>
            </div>
          </div>
