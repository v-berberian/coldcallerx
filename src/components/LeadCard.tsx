...
import CourtRecordsModal from './CourtRecordsModal';
...
const LeadCard: React.FC<LeadCardProps> = ({
  ...
  const [showCourtRecords, setShowCourtRecords] = useState(false);
  ...
  return (
    <>
      <Card className="shadow-2xl border-border/50 rounded-3xl bg-card h-[480px] flex flex-col">
        <CardContent className="p-8 space-y-6 flex-1 flex flex-col">
          ...
          <div className="space-y-3">
            ...
            {lead.company && (
              <Button 
                onClick={() => setShowCourtRecords(true)}
                variant="outline"
                size="sm"
      ...
      {/* Court Records Modal */}
      <CourtRecordsModal
        isOpen={showCourtRecords}
        onClose={() => setShowCourtRecords(false)}
